...
def add_glucose():
    ...
    if category in ["Hypoglycemia", "Hyperglycemia"]:
        ...
        cur3.execute("SELECT emergency_contact_name, emergency_contact_phone FROM patients WHERE user_id=%s", (user_id,))
        p = cur3.fetchone()
        emer_name = p['emergency_contact_name'] if p else ""
        emer_phone = p['emergency_contact_phone'] if p else ""

        # Doctor in-app message
        if doctor_id:
            alert_msg = f"ALERT: Patient {user_id} logged {category} ({glucose_level} mg/dL, {context}). Immediate attention may be needed."
            cur3.execute(
                "INSERT INTO messages (sender_id, receiver_id, message) VALUES (%s, %s, %s)",
                (user_id, doctor_id, alert_msg)
            )

        # Emergency contact SMS logic...
        # Emergency contact SMS
        if emer_phone:
            sms_body = f"ALERT: {category} detected for patient {user_id} ({glucose_level} mg/dL, {context}). Please check on them immediately."
            try:
                ...
                print(f"SMS sent to {emer_phone}: {sms_body}")
...
def update_medication(med_id):
    ...
    db = get_db()
    cur = db.cursor(dictionary=True)
    cur.execute("SELECT dosage FROM medications WHERE id=%s", (med_id,))
    old = cur.fetchone()
