...
class _DoctorProfilePageState extends State<DoctorProfilePage> {
  ...
  void fetchDoctorProfile() async {
    ...
    final res = await http.get(Uri.parse("http://192.168.100.53:5000/doctor_profile/${widget.doctorId}"));
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    if (loading) return Scaffold(appBar: AppBar(title: Text("Doctor Profile")), body: Center(child: CircularProgressIndicator()));
    if (doctorProfile == null) return Scaffold(appBar: AppBar(title: Text("Doctor Profile")), body: Center(child: Text("No profile data.")));
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                InfoRow(label: "License Number", value: doctorProfile?['license_number']),
                InfoRow(label: "City", value: doctorProfile?['city']),
                InfoRow(label: "Country", value: doctorProfile?['country']),
                SizedBox(height: 18),
                if (doctorProfile?['geo_lat'] != null && doctorProfile?['geo_lng'] != null)
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
